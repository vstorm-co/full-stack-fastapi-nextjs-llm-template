...
class ProjectConfig(BaseModel):
    ...
    def validate_option_combinations(self) -> "ProjectConfig":
        ...
        if self.enable_admin_panel and self.database == DatabaseType.MONGODB:
            raise ValueError("Admin panel (SQLAdmin) requires PostgreSQL, SQLite, or MS SQL Server")
        if self.orm_type == OrmType.SQLMODEL and self.database not in (
            DatabaseType.POSTGRESQL,
            DatabaseType.SQLITE,
            DatabaseType.SQLSERVER,
        ):
            raise ValueError("SQLModel requires PostgreSQL, SQLite, or MS SQL Server database")
